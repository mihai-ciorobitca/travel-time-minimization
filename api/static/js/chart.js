...
function titleArgs(scale, offset, position, align) {
  ...
  if (scale.isHorizontal()) {
    ...
    if (isObject(position)) {
      ...
      titleX =
        scales[positionAxisID1].getPixelForValue(value1) - width + offset;
    } else if (position === "center") {
...
class Chart {
  ...
  isPluginEnabled(pluginId) {
    return (
      this._plugins._cache.filter((p) => p.plugin.id === pluginId).length === 1
    );
  }
  ...
  getBody(tooltipItems, options) {
    ...
    each(tooltipItems, (context) => {
      ...
      pushOrConcat(
        bodyItem.before,
        splitNewlines(
          invokeCallbackWithFallback(scoped, "beforeLabel", this, context)
        )
      );
      pushOrConcat(
        bodyItem.lines,
      ...
      pushOrConcat(
        bodyItem.after,
        splitNewlines(
          invokeCallbackWithFallback(scoped, "afterLabel", this, context)
        )
      );
...
export {
  ...
  TimeScale,
  TimeSeriesScale$1 as TimeSeriesScale,
